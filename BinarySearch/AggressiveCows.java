...
public class AggressiveCows {
    ...
    // Function to find the maximum minimum distance between aggressive cows placed in stalls
    // The function takes an array of stall positions and a number of cows as input and returns the largest minimum distance
    // The function uses a binary search approach on the answer to solve the problem
    // The time complexity of the function is O(n log maxDistance), where n is the number of stalls. The space complexity is O(1) as we are using a constant amount of extra space.

    // Approach: Sort the stall positions. The answer lies between 1 and (max - min) distance.
    // Use binary search to find the maximum possible minimum distance.
    // For each mid value, check if we can place all cows with at least mid distance between them.
    // If feasible, try a larger distance (move left = mid + 1). Otherwise, try a smaller distance (move right = mid - 1).
    ...
    // Helper function to check if we can place all cows with at least minDist distance between them
