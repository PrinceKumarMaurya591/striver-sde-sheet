...
public class PeakElement {
    ...
    // Function to find a peak element in an array (element that is greater than its neighbors)
    // The function takes an array of integers as input and returns the index of any peak element
    ...
    // If nums[mid] > nums[mid + 1], there is a peak in the left half (including mid).
    // If nums[mid] < nums[mid + 1], there is a peak in the right half.
    // This works because the array boundaries are considered as -infinity, so a peak always exists.
    public int findPeakElement(int[] nums) {
        ...
        while (left < right) {
            ...
            if (nums[mid] > nums[mid + 1]) {
                // If mid is greater than its right neighbor, the peak is in the left half (including mid)
                right = mid;
            } else {
